...
|---|---|---|---|
| `TARS: turn N, context 52% (523k/1M), about 9 turns to auto-compact at this rate. No handoff written this session. Soft threshold (50%) reached. Handoff suggested.` | every | The context window crosses 50% full, then once per 5 points above that. Silent below 50%, however many turns. Read from the session transcript, so it matches the app's context ring. The turn estimate appears once the fill rate is known. Handoff status comes from the file on disk, not memory. A handoff this session wrote still counts after another session runs `/go` on it, judged by its `WRITTEN:` header. | Relay it, add anything contradictory you've noticed, good news or bad, and offer a handoff at the next task boundary. If the estimate says few turns remain, say so plainly. The human decides. Never withhold it. |
| `TARS: turn N, context 76% (...). ... Hard threshold (75%) reached.` | every | The context crosses 75%, then once per 5 points | Relay it, say plainly that auto-compact is close and recall is degrading, and write the handoff. |
| `TARS: context is already N% (...) after the first exchange. The boot layer is heavy.` | every | Once, when the first reading of a session is 15% or more | Relay it. The boot layer, firmware, or imports are costing the session before any work starts; name the likely culprit if you know it. |
| `TARS: turn N. No handoff written this session. Soft threshold (20) reached. Handoff suggested.` | every | Fallback only, when the transcript can't be read: turn 20, then every 5th turn; hard at 45 | Same duties as the context lines above. |
