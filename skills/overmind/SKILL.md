...
New in v4.1: orgs running more than one Overmind can form **the Collective** — no A2A server needed, just a shared folder (synced drive, git repo, or cloud connector). Team building's closing ceremony runs a one-time capability check for this (firmware TEAM BUILDING, soft gate — never blocks setup); full convene/seating mechanics live in `skills/collective/SKILL.md`.
