...
5. **Seat the peer** through the admission gate below. From here there's nothing left to do by hand — the firmware's Collective sweep (turn-based, running as a standing duty in whatever session either Overmind next happens to be used for) carries the gate forward one round at a time, same as everything else about ongoing Collective participation. No session needs to stay open for this; no human needs to relay a "check the collective" prompt between two people.
