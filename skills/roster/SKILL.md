...
6. **`COUNCIL_BOARD.md`** at the team root, when one exists — check the Seats table and any cross-team mission rows naming the affected member or this team's handle; update seat status, reassign or close CTM lanes, and log the change in the Event Log.
