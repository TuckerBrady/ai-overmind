...
3. **Boot layer** inside that folder (for adds/resurrections): `BOOT.md` (the canonical boot layer — single source for every runtime), the thin `CLAUDE.md` wrapper for working-directory runtimes (identity line + the `@BOOT.md` import — the filename is deliberately space-free; never rename it), and the `Project Instructions.md` paste-wrapper for paste-based runtimes. The persona lives inside BOOT.md as its `## Persona` section — there is no separate persona file, and a voice edit is a boot edit. A legacy `feedback_[name]_persona.md` gets folded into BOOT.md per the firmware's legacy persona migration (copied verbatim, old file stubbed, never deleted without asking). Boot edits go to BOOT.md only, never a wrapper — and by the dual-runtime law, a boot edit is not done until the Overmind has written BOOT.md's contents into any paste-based runtime itself.
   Every new or resurrected member's BOOT.md carries the `## How to work with [name]` section that imports `@../WORKING_WITH_[FIRSTNAME].md` (template in the firmware's BOOT.md template), and a short TARS relay line in RUNTIME ORIENTATION. Never paste the working-style rules into a member file; the import is the only copy. A RE-THEME never touches that file: it is about the human, not the team's names.
...
3. **Write the boot layer** per the firmware's TEAM BUILDING section — all at the folder root: `BOOT.md` (header naming it the canonical boot layer, `## Identity`, the `## Persona` section carrying the member's full voice, runtime orientation, the Sleeper Activation Protocol with Gopher Registration, Mission Complete Signal, Mission Board, and Inbox Protocol, any standing duties, and — only if the team root has a `TRANSPORT.md` — the A2A membership reflex), the thin `CLAUDE.md` wrapper (identity line + `@BOOT.md` import, space-free filename), the `Project Instructions.md` paste-wrapper (points at BOOT.md as the content the Overmind keeps in the platform's instructions; warns "Do not write instruction content here — it will drift and be lost"), and a starter INBOX.md.
4. **Add to the Sync Set** — roster table, dispatch roster, handoff voice archive, memory.
5. **Activate** — write the onboarding HANDOFF.md to the folder root (ACTIVATION block: activation is `/go`, no passphrase) and hand the human the standard activation instruction (new session → same team root → type `/go`). For a paste-based runtime, write BOOT.md's contents into that project's Project Instructions yourself before they open it. Track it on the status board until they're online.
...
3. **Walk the full Sync Set per member**, same as RENAME / RE-ROLE: folder name stays (role didn't change, just the persona name), bootstrap identity section, BOOT.md's `## Persona` section (update the Personality subsection's opening tone line to the new preset's), BOOT.md / CLAUDE.md wrapper / Project Instructions.md wherever the old name appears, GOPHER_REGISTRY.md rows, MISSION_BOARD.md rows, dispatch roster, INBOX.md headers, and Overmind memory. Note the old name in TEAM_ROSTER.md ("formerly [X]") for each, same as any rename.
4. **Update TEAM_ROSTER.md's Team Style header line** to the new preset (or "Freestyle").
5. **The dual-runtime law applies** — a renamed member's BOOT.md isn't done until the Overmind has updated the pasted instructions in any paste-based runtime that member runs in. Do it in the same pass for the whole batch; only a genuinely blocked runtime gets reported, once, as a blocker.
